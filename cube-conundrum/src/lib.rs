    ...
    fn from_lines(input: Vec<String>) -> Vec<Round> {
        ...
        for line in input {
            for x in line.split(", ") {
                ...
                rounds.push(Round { count, colour: Colour::from_string(comps[1]) });
            }
        }
...
impl Game {
    ...
    fn can_play(&self, other: &Game) -> bool {
         for other_round in &other.rounds {
            let self_round = self.rounds.clone().into_iter().find(|r| other_round.colour == r.colour ).unwrap();
    ...
    fn control_game() -> Game {
        return Game {
            id: 0,
            rounds: vec![
                Round { colour: Colour::RED, count: 12 },
                Round { colour: Colour::GREEN, count: 13 },
                Round { colour: Colour::BLUE, count: 14 },
            ],
            power: 0,
        }
    }
...
mod tests {
    ...
    fn can_play_game_three() {
        let control: Game = Game::control_game();
        let input: &str = "Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red";
        let game: Game = Game::from_string(input.to_string());
    ...
    fn sum_game_ids() {
        let games: Vec<Game> = vec![
            ...
            Game::from_string("Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue".to_string()),
            Game::from_string("Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red".to_string()),
            Game::from_string("Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red".to_string()),
